...
int main() {
    ...
    //     std::array<double, joint_count> angles{};   // OK
    //     static_assert(joint_count == 6);            // checked while compiling
    ...
    // --- [Slide 69] Global Scope ---------------------------------------------
    // Uncomment var at the top of this file first.
    ...
    //   {
    //     std::cout << var << '\n';  // 1
    //     var++;                     // any code here can change it: 2

    //     {
    //       int var{100};                // a LOCAL that shadows the global
    //       std::cout << var << '\n';    // 100  <- the local
    ...
    // --- [Slide 76] Deducing from the initializer ----------------------------
    // You write auto where the type would go; the compiler reads the
    // initializer to work the type out. It is still fixed and still checked.
    //   {
    //     auto count{10};     // int
    ...
    //     auto sum{1 + 2};    // 1 + 2 evaluates to an int -> sum is int
    //     auto copy{count};   // count is an int           -> copy is int

    //     std::cout << count << ' ' << ratio << ' ' << initial << ' ' << ready
    //               << ' ' << sum << ' ' << copy << '\n';
    //   }

    // --- [Slide 76] auto needs an initializer - DOES NOT COMPILE -------------
    //   {
    //     auto a;    // error: declaration of 'auto a' has no initializer
    //     auto b{};  // error: direct-list-initialization of 'auto' requires
    //                //        exactly one element
    //   }

    // --- [Slide 76] One value per auto declaration - DOES NOT COMPILE -------
    // Before C++17 this quietly gave you a std::initializer_list<int>.
    //   {
    //     auto x{1, 2};  // error: direct-list-initialization of 'auto'
    //   }                //        requires exactly one element

    // --- [Slide 77] What deduction throws away -------------------------------
    // The top-level const is dropped. Write const auto to ask for it back.
    //   {
    //     const int limit{5};

    //     auto copy{limit};        // int, the const has been dropped
    ...
    // --- [Slide 77] ... and const auto really is const - DOES NOT COMPILE ---
    //   {
    //     const int limit{5};
    //     const auto kept{limit};
    //     kept = 6;  // error: assignment of read-only variable 'kept'
    //   }

    // --- [Slide 78] When to use it -------------------------------------------
    // Reach for auto when the type is already written on the right. Write the
    // type out when the type IS the point of the line: the second declaration
    // below hides that speed is an int, and integer division is the bug.
    //   {
    //     auto reading{std::stod("11.4")};  // double, and you can see why

    //     auto speed{0};  // int, not double. Nothing on this line says so.
    ...
    // --- [Slide 80] An enumeration makes the values a type -------------------
    // A robot's state is one of a fixed set. Written as an int, nothing stops
    // state = 47. An enum class makes the set of values into a type.
    //   {
    ...
    // --- [Slide 80] ... and an int is not that type - DOES NOT COMPILE ------
    //   {
    //     enum class RobotState { idle, moving, charging };
    //     RobotState state{RobotState::idle};
    //     state = 1;  // error: cannot convert 'int' to 'RobotState' in
    //   }             //        assignment

    // --- [Slide 82] Scoped names stay out of each other's way ----------------
    //   {
    //     enum class Color { red, green };
    //     enum class LedState { red, off };  // fine: red is scoped to each type

    //     std::cout << static_cast<int>(Color::red) << ' '
    //               << static_cast<int>(LedState::red) << '\n';  // 0 0
    //   }

    // --- [Slide 82] An unscoped enum leaks its names - DOES NOT COMPILE -----
    //   {
    //     enum Color2 { red, green };
    //     enum LedState2 { red, off };  // error: 'red' conflicts with a
    //   }                               //        previous declaration

    // --- [Slide 82] Getting the number out -----------------------------------
    // static_cast is required: there is no implicit conversion. That absence
    // is what stops a state being compared against a speed.
    //   {
    //     enum class RobotState { idle, moving, charging };
    //     std::cout << static_cast<int>(RobotState::charging) << '\n';  // 2
    //   }

    // --- [Slide 83] Choosing the underlying type -----------------------------
    // int by default. On a message that goes over a network or a serial link,
    // say which one. Enumerators may be given explicit values, as Speed does.
    //   {
    //     enum class RobotState { idle, moving, charging };
    ...
    // --- [Slide 84] C++20: using enum ----------------------------------------
    // Drops the qualification inside ONE scope, here the body of the switch.
    // Leaving out default is deliberate: add an enumerator later and -Wswitch,
    // part of -Wall, warns about every switch that has not been updated.
    //   {
    //     enum class RobotState { idle, moving, charging };
    //     RobotState state{RobotState::idle};
    ...
    // --- [Slide 85] Predict first, then compile ------------------------------
